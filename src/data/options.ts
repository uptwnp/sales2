...
const personOptions: Option[] = [
  'Yogesh', 'Mohit', 'Sharvan', 'Lather Uncle', 'Suresh', 'Raju', 'Rinku', 'Pardeep', 'Narender', 'Other', 'Deepak', 'Komal'
].map(createOption);

// === Reusable Tags ===
const commonTags: Option[] = [
  'Meeting Done','Multiple Site Visit Done','First Sell then buy','Seller Also','Must Remember', 'Serious Buyer', 'Need Loan', 'VIP', 'Investor', 'May Be Investor', 'Repeat', 'International', 
  'Very Less Chances', 'Very Chances', 'Can Extend Budget', 'Special Case', 'Premium', 'Cash Buyer', 'First Time Buyer', 'VIP Client','to delete'
].map(createOption);
...
export const placementOptions = ['Main Page', 'Story', 'Search', 'Other'].map(createOption);
export const nextActionOptions = ['Call Back', 'Schedule Visit', 'Follow Up', 'Meeting', 'Other'].map(createOption);
export const purposeOptions = ['Self Use', 'Investment', 'Rental', 'Resale', 'Bussiness', 'Other'].map(createOption);
export const todoTypeOptions = ['Follow Up', 'Meeting', 'Find Match', 'Schedule Site Visit', 'Todo','Activity', 'Other'].map(createOption);
...
export function getOptionByApiValue(options: Option[], apiValue: string): Option | undefined {
